...
SELECT 
    ...
    total_net_change
FROM company_totals
ORDER BY 1, 2; -- Conclusion: Tech workforce growth followed a synchronized hyper-expansion during the pandemic, led by FAANG, followed by a sharp industry-wide correction beginning in 2022


-- 4. Changes in hiring rate, attrition rate and GDP growth (US) based on company group during major economical periods
...
ORDER BY
    ...
    END,
    company_group; -- Conclusion: Hiring peaked during the early digital expansion cycles, while the pandemic period further accelerated workforce growth across all groups. 
                   -- The AI expansion represents a structural shift, with tech sector growth now driven more by productivity than by headcount

-- 5. Changes in total comapny revenue and weighted avg revenue per employee based on company group during major economical periods
WITH company_group_creation AS (
    SELECT 
        CASE
            WHEN company IN ('Meta', 'Apple', 'Amazon', 'Netflix', 'Alphabet') THEN 'FAANG'
...
ORDER BY
    CASE economic_period
        ...
        WHEN 'The AI Expansion' THEN 5
    END,
    company_group; -- Conclusion: Tech companies are generating record revenue per employee during the AI Expansion period, signaling a shift from workforce-driven growth to productivity-driven growth.


-- 6. How does the market react to workforce changes?
--Finding how does the workforce changes affect company revenue, revenue per employee and stock price per company group
...
SELECT
    ...
    company_group,
    COUNT(*) AS case_count,
    ROUND(AVG(attrition_rate_pct), 2) AS avg_attrition,
...
GROUP BY stock_performance_category, company_group
ORDER BY 
    CASE stock_performance_category
        ...
        WHEN 'Crash' THEN 8
    END,
    company_group -- Conclusion: Attrition doesn't matter, it's flat across every category. Hiring rate is actually a warning sign, not a strength; 
                  -- Newer/Gig Economy companies hired most aggressively right before their worst stock years. 
                  -- Revenue per employee is the real predictor: higher efficiency consistently correlates with better stock outcomes, with FAANG leading at $1M–$1.9M per employee vs Established Tech at $450K–$850K

    
-- 7. How the Workforce Volatility was changing in different Tech Groups Across Economic Cycles
--Note: Volatility metrics for the Newer/Gig Economy group prior to 2005 are unavailable/zero due to the limited number of companies established in that sector at the time
WITH company_group_creation AS (
    SELECT 
        CASE
            WHEN company IN ('Meta', 'Apple', 'Amazon', 'Netflix', 'Alphabet') THEN 'FAANG'
...
ORDER BY
    CASE economic_period
        ...
        WHEN 'The AI Expansion' THEN 5
    END,
    company_group; -- Conclusion: Newer companies chase growth, but established giants provide stability. 
...
WITH b1 AS (
    ...
    JOIN tech_wf t2
        ON t1.company = t2.company
        AND t2.year = t1.year + 1 
	ORDER BY next_year_stock_change ASC, next_year_revenue_per_employee DESC
 ),
